...
type PackageRecord = {
  ...
  sizeBeforeCompressionBytes: number;
  sizeAfterCompressionBytes: number;
...
function formatBytesFromApi(value: unknown) {
  const bytes = numberFromApi(value, 0);
...
function formatCompressionRatio(beforeBytes: number, afterBytes: number) {
  if (!beforeBytes || beforeBytes <= 0 || !afterBytes || afterBytes <= 0) return "-";
  return `${((afterBytes / beforeBytes) * 100).toFixed(2)}%`;
}

function formatRemoteExecuteValue(value: number) {
  if (!Number.isFinite(value) || value === 0) return "Not configured";
  return `${value} (DB flag)`;
}

function formatExcludeOsValue(value: number) {
...
function normalizePackageRecord(row: any): PackageRecord {
  ...
  const name = firstDefined(row.name, row.pkg_Name, row.Pkg_Name, row.PKG_Name, row.packageName, raw.Pkg_Name, raw.PKG_Name, "-");
  const id = String(firstDefined(row.id, row.pkg_Idn, row.Pkg_Idn, row.PKG_Idn, raw.Pkg_Idn, raw.PKG_Idn, name));
  const versionValue = firstDefined(row.version, row.pkg_Version, row.Pkg_Version, row.PKG_Version, raw.Pkg_Version, raw.PKG_Version, 1);
  const deletedFlag = numberFromApi(firstDefined(row.pkg_bDeleted, row.Pkg_bDeleted, raw.Pkg_bDeleted), 0);
  const statusValue = firstDefined(row.status, row.Status, deletedFlag === 1 ? "Archived" : "Ready");
  const status: PackageStatus = ["Ready", "Draft", "Deployed", "Archived"].includes(String(statusValue))
    ? String(statusValue) as PackageStatus
    : "Ready";

  const rawVersions = firstDefined(row.versions, row.pkg_AllVersions, row.Pkg_AllVersions, row.allVersions, row.AllVersions, raw.AllVersions);
  const versions = Array.isArray(rawVersions)
    ? rawVersions.map((item) => String(item).startsWith("VER_") ? String(item) : `VER_${item}`)
    : String(rawVersions || versionValue)
      .split(",")
      .map((item) => item.trim())
      .filter(Boolean)
      .map((item) => item.startsWith("VER_") ? item : `VER_${item.replace(/^v/i, "")}`);

  const sizeBeforeBytes = numberFromApi(firstDefined(
    row.sizeBeforeCompression,
    row.SizeBeforeCompression,
    row.sizeBefore,
    row.SizeBefore,
    raw.SizeBeforeCompression,
    row.Pkg_File_OSize
  ), 0);

  const sizeAfterBytes = numberFromApi(firstDefined(
    row.sizeAfterCompression,
    row.SizeAfterCompression,
    row.sizeAfter,
    row.SizeAfter,
    raw.SizeAfterCompression,
    row.Pkg_File_ZSize
  ), 0);

  const savedBytes = Math.max(sizeBeforeBytes - sizeAfterBytes, 0);

  return {
    id,
    name: String(name),
    version: String(versionValue).startsWith("v") ? String(versionValue) : `v${versionValue}`,
    description: String(firstDefined(row.description, row.pkg_Description, row.PKG_Info, row.Pkg_Info, raw.Pkg_Info, raw.PKG_Info, "-")),
    status,
    owner: String(firstDefined(row.owner, row.pkg_Owner, row.Pkg_Owner, raw.Pkg_Owner, "system")),
    destinationDirectory: String(firstDefined(row.destinationDirectory, row.pkg_File_CTDir, row.Pkg_File_CTDir, row.destination_path, "-")),
    registeredDate: normalizeApiDate(firstDefined(row.registeredDate, row.pkg_ChangedDate, row.Pkg_ChangedDate, raw.Pkg_ChangedDate, row.createdAt)),
    fileCount: numberFromApi(firstDefined(row.fileCount, row.FileCount, row.file_count, raw.FileCount), 0),
    sizeBeforeCompression: formatBytesFromApi(sizeBeforeBytes),
    sizeAfterCompression: formatBytesFromApi(sizeAfterBytes),
    sizeBeforeCompressionBytes: sizeBeforeBytes,
    sizeAfterCompressionBytes: sizeAfterBytes,
    compressionSaved: savedBytes > 0 ? formatBytesFromApi(savedBytes) : "-",
    compressionRatio: formatCompressionRatio(sizeBeforeBytes, sizeAfterBytes),
    osCode: String(firstDefined(row.osCode, row.Pkg_OS, row.pkg_OS, raw.Pkg_OS, raw.PKG_OS, "-")),
    excludeOS: numberFromApi(firstDefined(row.excludeOS, row.Exclude, row.exclude, raw.Exclude), 0),
    remoteExecuteFile: numberFromApi(firstDefined(row.remoteExecuteFile, row.RemoteExecuteFile, raw.RemoteExecuteFile), 0),
    lastDeployment: normalizeApiDate(firstDefined(row.lastDeployment, row.LastDeployment)),
    targetCount: numberFromApi(firstDefined(row.targetCount, row.TargetCount), 0),
    versions: versions.length ? versions : [`VER_${String(versionValue).replace(/^v/i, "")}`],
    lastDeliveryMethod: firstDefined(row.lastDeliveryMethod, "-") as DeliveryMethod | "mixed" | "-",
  };
}
...
export default function SoftwareDistribution() {
  ...
  const exportCsv = () => {
    ...
    const rows = filteredPackages.map((item) => [
      ...
      item.sizeAfterCompression,
      item.compressionSaved,
      item.compressionRatio,
      item.remoteExecuteFile,
      item.excludeOS,
    ]);
              ...
              <table className="table">
                <thead>
                  <tr>
                    ...
                    <th><SortButton label="Files" columnKey="fileCount" /></th>
                  ...
                  {pageRows.map((item, index) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <td>{item.sizeAfterCompression}</td>
                        <td>{item.owner}</td>
                        <td onClick={(event) => event.stopPropagation()}>
                          <button
                            type="button"
          ...
          <aside className="user-modal advanced w-75" onMouseDown={(event) => event.stopPropagation()}>
            ...
            <div className="user-modal-body content-body gap-3">
              <section className="wide audit-kpi-strip gap-3">
                ...
                <div className="score-box"><span>Files:</span><strong>{selectedPackage.fileCount}</strong><small>Total files in package</small></div>
                <div className="score-box"><span>Compressed Size:</span><strong>{selectedPackage.sizeAfterCompression}</strong><small>Stored package size</small></div>
                <div className="score-box"><span>Saved:</span><strong>{selectedPackage.compressionSaved}</strong><small>Compression saving</small></div>
              </section>

              <section className="policy-card wide p-4">
                <div className="pricing-top"><div><h4>Basic Information</h4><p>Package identity and ownership details</p></div></div>
                <div className="pricing-grid gap-3">
                  <label className="form-field"><span>Package ID:</span><strong>{selectedPackage.id}</strong></label>
                  ...
                  <label className="form-field"><span>OS Code:</span><strong>{selectedPackage.osCode}</strong></label>
              ...
              <section className="policy-card wide p-4">
                ...
                <div className="pricing-grid gap-3">
                  ...
                  <label className="form-field"><span>Original Size:</span><strong>{selectedPackage.sizeBeforeCompression}</strong></label>
                  <label className="form-field"><span>Compressed Size:</span><strong>{selectedPackage.sizeAfterCompression}</strong></label>
                  <label className="form-field"><span>Compression Saved:</span><strong>{selectedPackage.compressionSaved}</strong></label>
                  <label className="form-field"><span>Compression Ratio:</span><strong>{selectedPackage.compressionRatio}</strong></label>
                  <label className="form-field"><span>Remote Execute:</span><strong>{formatRemoteExecuteValue(selectedPackage.remoteExecuteFile)}</strong></label>
                  <label className="form-field"><span>Exclude OS:</span><strong>{formatExcludeOsValue(selectedPackage.excludeOS)}</strong></label>
                </div>
              </section>
            </div>
