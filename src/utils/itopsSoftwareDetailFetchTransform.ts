...
const SOFTWARE_DETAIL_FETCH_EFFECT = String.raw`  useEffect(() => {
  ...
  useEffect(() => {
    ...
    const terms = (() => {
      ...
      if (key.includes('antivirus')) return ['defender', 'sophos', 'symantec', 'mcafee', 'trellix', 'kaspersky', 'crowdstrike', 'sentinelone', 'bitdefender', 'eset', 'malwarebytes', 'forticlient'];
      if (key.includes('remote')) return ['anydesk', 'teamviewer', 'vnc', 'rustdesk', 'dameware', 'logmein', 'splashtop', 'screenconnect', 'connectwise', 'radmin'];
      if (key.includes('browser') || key.includes('web')) return ['chrome', 'firefox', 'edge', 'brave', 'opera', 'safari', 'vivaldi', 'chromium'];
      if (key.includes('gaming') || key.includes('game')) return ['steam', 'epic games', 'riot', 'valorant', 'garena', 'battle.net', 'blizzard', 'ea app', 'origin', 'ubisoft', 'roblox', 'minecraft', 'xbox'];
      return [];
    })();
    ...
    const loadRows = async () => {
      ...
      try {
        ...
        for (const term of terms) {
          const response = await fetch(buildApiUrl('/api/software', { search: term, limit: 100 }), { headers, credentials: 'include' });
          if (!response.ok) continue;
          const payload = await response.json().catch(() => null) as any;
          const items = Array.isArray(payload?.data?.items) ? payload.data.items : Array.isArray(payload?.items) ? payload.items : Array.isArray(payload?.data) ? payload.data : [];
