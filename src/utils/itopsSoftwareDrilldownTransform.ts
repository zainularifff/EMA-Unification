...
const ENHANCED_SOFTWARE_HELPERS = String.raw`
  ...
  const softwareMajorFamilyMatches = (row: SoftwareInventoryRow, family: string) => {
    ...
    if (family === 'Microsoft Office') return /microsoft\s+office|\boffice\s*(2016|2019|2021|2024)?\b|\bword\b|\bexcel\b|powerpoint|outlook|access|visio|project/.test(text) && !/microsoft\s*365|office\s*365/.test(text);
  ...
  const softwareClassificationMatches = (row: SoftwareInventoryRow, selected: string) => {
    const text = getSoftwareRowSearchText(row);
    if (selected.includes('business')) return /business|productivity|microsoft|office|microsoft\s*365|adobe|acrobat|autocad|sap|oracle|account|finance|hr|payroll|visio|project/.test(text);
    if (selected.includes('remote')) return /remote|teamviewer|anydesk|vnc|ultravnc|realvnc|tightvnc|rustdesk|dameware|bomgar|beyondtrust|screenconnect|connectwise|radmin/.test(text);
    if (selected.includes('antivirus') || selected.includes('anti-virus')) return /antivirus|anti-virus|endpoint protection|defender|sophos|symantec|mcafee|kaspersky|trend micro|crowdstrike|sentinelone|bitdefender|eset|avast|malwarebytes/.test(text);
    ...
    if (selected.includes('gaming') || selected.includes('game')) return /game|gaming|steam|epic games|riot|valorant|garena|battle.net|blizzard|ea app|origin|ubisoft|roblox/.test(text);
  ...
  const renderSoftwareStackedDistribution = (items: { label: string; value: number; target: string; tone: CardTone }[], total: number) => {
    ...
    return (
      <div style={{ display: 'grid', gap: 16 }}>
        <div style={{ display: 'flex', height: 32, overflow: 'hidden', borderRadius: 999, border: '1px solid #dbeafe', background: '#f8fafc', boxShadow: 'inset 0 1px 2px rgba(15,23,42,.06)' }}>
          {items.filter((item) => numberOrFallback(item.value) > 0).map((item) => {
            const share = Math.max(1.5, (numberOrFallback(item.value) / safeTotal) * 100);
            return (
              <button
                key={item.label}
                type="button"
                title={item.label + ' · ' + formatNumber(item.value)}
                onClick={() => openLevel3('software', item.target)}
                style={{ width: String(share) + '%', minWidth: 18, border: 0, borderRight: '1px solid rgba(255,255,255,.55)', background: toneGradient(item.tone), cursor: 'pointer' }}
              />
            );
          })}
        </div>
          ...
          {items.map((item) => {
            ...
            return (
              <button
                ...
                type="button"
                onClick={() => openLevel3('software', item.target)}
                style={{ display: 'grid', gap: 6, padding: 12, border: '1px solid #e2e8f0', borderRadius: 16, background: toneSoftBg(item.tone), textAlign: 'left', color: '#0f172a', cursor: 'pointer' }}
              >
                <span style={{ display: 'flex', alignItems: 'center', gap: 8, color: '#64748b', fontSize: 10, fontWeight: 950, letterSpacing: '.08em', textTransform: 'uppercase' }}>
                  <i style={{ width: 9, height: 9, borderRadius: 999, background: toneSolid(item.tone), boxShadow: '0 0 0 4px rgba(148,163,184,.14)' }} />
                  {item.label}
                </span>
                <strong style={{ fontSize: 22, lineHeight: 1, fontWeight: 950 }}>{formatNumber(value)}</strong>
                <span style={{ color: '#475569', fontSize: 12, fontWeight: 850 }}>{formatPercent(share, 1)} of software estate</span>
              </button>
            );
          })}
        </div>
        ...
        {items.map((item) => {
          ...
          return (
            <button
              ...
              key={item.label}
              onClick={() => openLevel3('software', item.target)}
              style={{ display: 'grid', gridTemplateColumns: '160px minmax(0, 1fr) 92px', alignItems: 'center', gap: 12, width: '100%', padding: '10px 12px', border: '1px solid #e2e8f0', borderRadius: 16, background: '#fff', color: '#0f172a', textAlign: 'left', cursor: 'pointer' }}
            >
              <div style={{ minWidth: 0 }}>
                <strong style={{ display: 'block', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', fontSize: 13, fontWeight: 950 }}>{item.label}</strong>
                <span style={{ display: 'block', marginTop: 3, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', color: '#64748b', fontSize: 11, fontWeight: 800 }}>{item.note}</span>
              </div>
              <div style={{ position: 'relative', height: 18, overflow: 'hidden', borderRadius: 999, background: '#e2e8f0' }}>
                <i style={{ display: 'block', width: String(width) + '%', height: '100%', borderRadius: 999, background: toneGradient(item.tone), boxShadow: '0 8px 18px rgba(37,99,235,.16)' }} />
              </div>
              <div style={{ textAlign: 'right' }}>
                <strong style={{ display: 'block', fontSize: 17, fontWeight: 950 }}>{formatNumber(value)}</strong>
                <span style={{ color: '#64748b', fontSize: 10, fontWeight: 850 }}>{formatPercent(share, 1)}</span>
              </div>
            </button>
          );
        })}
  ...
  const renderSoftwareLifecycleDonut = (items: { label: string; value: number; target: string; note: string; tone: CardTone }[], total: number) => {
    ...
    return (
      <div style={{ display: 'grid', gridTemplateColumns: '180px minmax(0, 1fr)', gap: 18, alignItems: 'center' }}>
        <button
          type="button"
          onClick={() => openLevel3('software', watchCount > 0 ? 'EOL/EOS Watch' : 'Supported')}
          style={{ width: 170, height: 170, border: '1px solid #e2e8f0', borderRadius: '50%', background: 'conic-gradient(' + (gradientParts || '#e2e8f0 0deg 360deg') + ')', display: 'grid', placeItems: 'center', cursor: 'pointer', boxShadow: '0 18px 45px rgba(15,23,42,.10)' }}
        >
          <span style={{ width: 104, height: 104, borderRadius: '50%', background: '#fff', display: 'grid', placeItems: 'center', textAlign: 'center', boxShadow: 'inset 0 0 0 1px #e2e8f0' }}>
            <strong style={{ display: 'block', fontSize: 25, lineHeight: 1, fontWeight: 950, color: '#0f172a' }}>{formatPercent(riskShare, 0)}</strong>
            <small style={{ color: '#64748b', fontSize: 10, fontWeight: 900, textTransform: 'uppercase', letterSpacing: '.08em' }}>Exposure</small>
          </span>
        </button>
            ...
            return (
              <button key={item.label} type="button" onClick={() => openLevel3('software', item.target)} style={{ display: 'grid', gridTemplateColumns: '1fr auto', gap: 10, alignItems: 'center', border: '1px solid #e2e8f0', borderRadius: 14, background: '#fff', padding: '10px 12px', cursor: 'pointer', color: '#0f172a', textAlign: 'left' }}>
                <span style={{ minWidth: 0 }}>
                  <strong style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 12, fontWeight: 950 }}><i style={{ width: 9, height: 9, borderRadius: 999, background: toneSolid(item.tone) }} />{item.label}</strong>
    ...
    return (
      <div style={{ display: 'grid', gap: 16 }}>
        <div style={{ display: 'grid', gridTemplateColumns: 'minmax(0, 1.1fr) minmax(360px, .9fr)', gap: 16, alignItems: 'stretch' }}>
          <Panel title="Software Classification Distribution" subtitle="Portfolio split by business, remote control, antivirus, browser, gaming and unclassified records. Click any segment to open the matching list." icon={BarChart3}>
            {renderSoftwareStackedDistribution(classificationRows, totalInstallations)}
          </Panel>
          <Panel title="Lifecycle Exposure" subtitle="Application support posture from lifecycle lookup. The donut highlights EOL/EOS and unsupported exposure." icon={ShieldAlert}>
            {renderSoftwareLifecycleDonut(lifecycleRows, totalInstallations)}
          </Panel>
        </div>

        <div style={{ display: 'grid', gridTemplateColumns: 'minmax(0, 1fr) minmax(0, 1fr)', gap: 16, alignItems: 'stretch' }}>
          <Panel title="Major Application EOL/EOS Watch" subtitle="Microsoft Office, Microsoft 365, Adobe, Google Chrome and Firefox coverage with click-through detail." icon={CalendarDays}>
            {renderSoftwareHorizontalBars(majorRows, totalInstallations)}
          </Panel>
          <Panel title="Software Governance Balance" subtitle="Shows the relationship between classified inventory, cleanup backlog and lifecycle risk." icon={Gauge}>
            {renderSoftwareHorizontalBars(governanceRows, totalInstallations)}
          </Panel>
        </div>
      </div>
    );
  ...
  const renderSoftwareLevel2Analytics = () => (
    <div className="itops-pro-drawer-stack">
      ...
      <div className="itops-pro-story-panel">
        <strong>Software Estate Dashboard</strong>
        <p>Start with the graphs below. They show classification distribution, major application lifecycle exposure, and software governance balance. Click any segment or bar to open the related software list.</p>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, minmax(0, 1fr))', gap: 12 }}>
        ...
        <DrillCard icon={Database} label="Unique Software" value={formatNumber(software.uniqueSoftware)} note="Unique software titles" tone="blue" onClick={() => openLevel3('software', 'Unique Software')} />
        <DrillCard icon={AlertTriangle} label="Cleanup Backlog" value={formatNumber(software.unclassifiedSoftware)} note="Unclassified records" tone="amber" onClick={() => openLevel3('software', 'Unclassified')} />
        <DrillCard icon={ShieldAlert} label="Lifecycle Watch" value={formatNumber(numberOrFallback(software.eolApplications) + numberOrFallback(software.eosApplications) + numberOrFallback(software.unsupportedApplications))} note="EOL/EOS and unsupported" tone="red" onClick={() => openLevel3('software', 'EOL/EOS Watch')} />
      </div>

      {renderSoftwareAnalyticsGraphs()}

      <Panel title="Application Lifecycle Detail Cards" subtitle="Raw lifecycle signals returned by the service lookup for monitored major applications." icon={CalendarDays}>
