...
import { Activity, AlertCircle, CheckCircle2, ChevronDown, ChevronLeft, ChevronRight, ChevronsLeft, ChevronsRight, Database, Edit3, Eye, Folder, FolderOpen, FolderPlus, Loader2, Monitor, MoreVertical, Network, Play, Plus, RefreshCw, Router, Search, ShieldCheck, Trash2, X, Zap, } from "lucide-react";
...
const API_BASE_URL = String((import.meta as unknown as {
    env?: Record<string, string>;
}).env?.VITE_API_BASE_URL || "http://localhost:3001").replace(/\/+$/, "");
...
function NetworkCustomSelect({ value, options, onChange, ariaLabel, }: {
    value: string;
    ...
    const menuNode = open && typeof document !== "undefined" ? createPortal(<div ref={menuRef} role="listbox" aria-label={ariaLabel}>
      {options.map((option) => {
    ...
    return (<div>
      <button ref={triggerRef} type="button" aria-label={ariaLabel} aria-expanded={open} onClick={() => setOpen((current) => !current)}>
...
function normalizeManualStatus(value: unknown): ManualDeviceStatus {
    ...
    if (text.includes("inactive") || text.includes("offline"))
        return "Inactive";
    if (text.includes("maintenance") || text.includes("review"))
        return "Maintenance";
    return "Active";
}
...
function getNetworkBranchLabel(node?: Pick<NetworkHierarchyNode, "id" | "label"> | null) {
    ...
    if (!label || normalized === "organization" || normalized === "organisation" || node?.id === "organization")
        return "All Network";
    return label;
}
...
function countIpSegments(node: NetworkHierarchyNode | null, depth = 0) {
    ...
    for (const child of node.children || [])
        total += countIpSegments(child, depth + 1);
    return total;
}
...
function collectIpTargets(node?: NetworkHierarchyNode | null): string[] {
    ...
    (Object.keys(emptyCounts) as CountKey[]).forEach((key) => {
        (node.deviceDetails?.[key] || []).forEach((detail) => {
            if (detail.ipAddress && isIpAddress(detail.ipAddress))
                ips.push(detail.ipAddress);
        });
    });
    for (const child of node.children || [])
        ips.push(...collectIpTargets(child));
    return Array.from(new Set(ips));
}
function rowsFromObject(row: Record<string, unknown> | undefined) {
    if (!row)
        return [];
    return Object.entries(row).map(([key, value]) => [key, getString(value, "-")] as [
        string,
        string
    ]);
}
...
function buildIpDetailRows(ip: string, node: NetworkHierarchyNode | null, agentRow?: Record<string, unknown>, objectRow?: Record<string, unknown>): Array<[
    string,
    string
]> {
...
export default function NetworkInventory() {
    ...
    const loadManualDevices = useCallback(async () => {
        ...
        try {
            const response = await apiRequest<Record<string, unknown>[]>(`/api/network/network-device-status${toQuery({ page: 1, limit: 500, search: manualSearch })}`);
    ...
    const loadInventory = useCallback(async (silent = false) => {
        ...
        try {
            const [hierarchyResult, dateResult, workgroupResult] = await Promise.allSettled([
                ...
                apiRequest<{
                    LastSearchDateStr?: string;
                }>("/api/network/search-date"),
    ...
    const openStatusDetails = async (type: CountKey, nextPage = 1) => {
        ...
        try {
            if (canCallSubnetApi) {
                const response = await apiRequest<DeviceDetail[]>(`/api/network/subnet/${encodeURIComponent(selectedNode.label)}/details${toQuery({ type, page: nextPage, limit: statusDetailPageSize })}`);
                setStatusDetail({
                    type,
                    title,
    ...
    return (<main data-section="users">
          ...
          <div>
            <div>
              {(<>
                  <div>
                    <Search size={15}/>
                    <input placeholder="Search IP / subnet..." value={treeSearch} onChange={(event) => {
                ...
                setFolderMenuId(null);
                  ...
                  <div aria-label="Network IP and subnet tree">
                    {loading ? (<div><Loader2 size={14}/> Loading network hierarchy...</div>) : filteredHierarchy ? (<div>
                        <NetworkTree key={treeSearch.trim() ? `search-${treeSearch.trim()}` : hierarchy?.id || "network-tree"} node={filteredHierarchy} selectedNodeId={selectedNode?.id || null} expandedIds={expandedTreeIds} onToggle={handleToggleTreeNode} onSelect={handleSelectNode} folderMenuId={folderMenuId} onMenu={setFolderMenuId} onAddFolder={openAddFolderDialog} forceOpen={Boolean(treeSearch.trim())}/>
                      </div>) : (<div>No IP segment found.</div>)}
                  </div>
                </>)}
            </div>
          </div>
          ...
          <main>
            ...
            <div>
              {activeTab === "network" ? (<>
                  <label>
                    <Search size={15}/>
                    <input placeholder="Search device, brand, location..." value={manualSearch} onChange={(event) => setManualSearch(event.target.value)}/>
                  </label>

                  <NetworkCustomSelect value={manualStatusFilter} options={[
                { value: "All", label: "All status" },
                { value: "Active", label: "Active" },
                { value: "Inactive", label: "Inactive" },
                { value: "Maintenance", label: "Maintenance" },
            ]} ariaLabel="Filter network device status" onChange={(nextValue) => setManualStatusFilter(nextValue as "All" | ManualDeviceStatus)}/>
                </>) : (<div>
                  <strong>Reference: {lastSearchDate}</strong>
                </div>)}

              <button type="button" onClick={() => activeTab === "device" ? void loadInventory(true) : void loadManualDevices()} disabled={refreshing || manualLoading || busy}>
                {(refreshing || manualLoading) ? <Loader2 size={15}/> : <RefreshCw size={15}/>}
                Refresh
              </button>

              {activeTab === "device" ? (<button type="button" onClick={() => openScanDialog(selectedNode?.id === hierarchy?.id ? "all" : isIpAddress(selectedNode?.label) ? "ip" : "subnet", selectedNode)} disabled={!selectedNode || busy}>
                  <Zap size={15}/>
                  Scan Network
                </button>) : (<button type="button" onClick={openAddDevice} disabled={busy}>
                  <Plus size={16}/>
                  Add New
                </button>)}
            </div>
            ...
            <div>
              {activeTab === "device" ? (ipDetail ? (<NetworkPathDetail detail={ipDetail} onScan={() => openScanDialog("ip", selectedNode)}/>) : (<DeviceStatusOverview counts={selectedCounts} total={countTotal(selectedCounts)} selectedLabel={getNetworkBranchLabel(selectedNode)} targetCount={selectedIps.length} onOpenStatus={(type) => void openStatusDetails(type)}/>)) : (<>
                  <DeviceRegistryTable rows={manualPageRows} page={manualPage} selectedId={editingDevice?.id || null} loading={manualLoading} onEdit={openEditDevice} onDelete={(device) => setDeleteTarget(device)}/>
      ...
      {(isFormOpen || editingDevice) && (<DeviceFormModal mode={editingDevice ? "edit" : "add"} device={editingDevice} busy={busy} onClose={() => {
                setIsFormOpen(false);
                setEditingDevice(null);
            }} onSave={(payload) => void handleSaveDevice(payload)}/>)}

      {deleteTarget && (<DeleteDeviceConfirmModal device={deleteTarget} busy={busy} onCancel={() => {
                if (!busy)
                    setDeleteTarget(null);
            }} onConfirm={() => void handleDeleteDevice(deleteTarget)}/>)}

      {addFolderDialog && (<AddFolderModal parentLabel={addFolderDialog.parentLabel} busy={busy} onClose={() => setAddFolderDialog(null)} onSave={handleAddFolder}/>)}

      {statusDetail && (<StatusDetailModal detail={statusDetail} onClose={() => setStatusDetail(null)} onPage={(nextPage) => void openStatusDetails(statusDetail.type, nextPage)} onOpenRecord={(row) => void openBackendRecord(row)}/>)}
...
function DeleteDeviceConfirmModal({ device, busy, onCancel, onConfirm, }: {
    device: ManualNetworkDevice;
    busy: boolean;
    onCancel: () => void;
    onConfirm: () => void;
}) {
    return (<div onMouseDown={busy ? undefined : onCancel}>
...
function NetworkTree({ node, level = 0, selectedNodeId, expandedIds, onToggle, onSelect, folderMenuId, onMenu, onAddFolder, forceOpen = false, }: {
    ...
    return (<div>
      ...
      {hasChildren && isOpen && (<div>
          {(node.children || []).map((child) => (<NetworkTree key={child.id} node={child} level={level + 1} selectedNodeId={selectedNodeId} expandedIds={expandedIds} onToggle={onToggle} onSelect={onSelect} folderMenuId={folderMenuId} onMenu={onMenu} onAddFolder={onAddFolder} forceOpen={forceOpen}/>))}
        </div>)}
    </div>);
}
function WorkgroupStatisticList({ rows, selected, onSelect }: {
    rows: WorkgroupStat[];
    selected: string;
    ...
    return (<div>
      {rows.map((row) => (<button key={row.name} type="button" onClick={() => onSelect(row.name)}>
          <span><Database /></span>
          <span>
...
function DeviceStatusOverview({ counts, total, selectedLabel, targetCount, onOpenStatus, }: {
        ...
        <table>
          ...
          <tbody>
            ...
            return (<tr key={row.type}>
                  ...
                  <td>
                    <button type="button" onClick={() => onOpenStatus(row.type)} aria-label={row.actionLabel} title={row.actionLabel}>
                      <Eye size={15}/>
                    </button>
                  </td>
                </tr>);
...
function StatusDetailModal({ detail, onClose, onPage, onOpenRecord, }: {
...
function StatusDetailTable({ detail, onPage, onOpenRecord, }: {
    ...
    useEffect(() => {
        if (!detail?.serverPaginated)
            setClientPage(detail?.page || 1);
    ...
    return (<div>
      ...
      <div>
        <label>
          <Search size={16}/>
          <input value={searchText} onChange={(event) => setSearchText(event.target.value)} placeholder="Search computer, user, IP, department..."/>
          {searchText && (<button type="button" onClick={() => setSearchText("")} aria-label="Clear detail search">
              <X size={14}/>
            </button>)}
        </label>
        ...
        <select value={agentFilter} onChange={(event) => setAgentFilter(event.target.value)}>
          ...
          {agentOptions.map((option) => (<option value={option} key={option}>Agent {option}</option>))}
        </select>

        <select value={workgroupFilter} onChange={(event) => setWorkgroupFilter(event.target.value)}>
          <option value="All">All workgroups</option>
          {workgroupOptions.map((option) => (<option value={option} key={option}>{option}</option>))}
        </select>

        {isFiltering && (<button type="button" onClick={clearFilters}>
            Clear
          </button>)}
      ...
      <div>
        <table>
          ...
          <tbody>
            {visibleRows.map((row, index) => (<tr key={`${row.ipAddress || row.computerName || "row"}-${activePage}-${index}`}>
            ...
            {!detail.loading && !visibleRows.length && (<tr>
                <td colSpan={8}>
...
function NetworkPathDetail({ detail, onScan }: {
    detail: IpDetailState;
    onScan: () => void;
}) {
    if (!detail)
        return null;
    return (<div>
      ...
      <div>
        <table>
          ...
          <tbody>
            {detail.rows.map(([label, value]) => (<tr key={label}>
                <td>{label}</td>
                <td>{value || "-"}</td>
              </tr>))}
          </tbody>
...
function DeviceRegistryTable({ rows, page, selectedId, loading, onEdit, onDelete, }: {
    ...
    return (<div>
      <table>
        ...
        <tbody>
          {rows.map((device, index) => (<tr key={device.id}>
...
function Pagination({ page, totalPages, totalRows, pageSize, itemLabel = "records", onChange, }: {
    page: number;
    totalPages: number;
    totalRows: number;
    pageSize?: number;
...
function AddFolderModal({ parentLabel, busy, onClose, onSave, }: {
    parentLabel: string;
    busy: boolean;
    onClose: () => void;
...
function DeviceFormModal({ mode, device, busy, onClose, onSave, }: {
    mode: "add" | "edit";
...
function RecordDetailModal({ detail, onClose }: {
    detail: NonNullable<RecordDetailState>;
    onClose: () => void;
}) {
    return (<div onMouseDown={onClose}>
      <section onMouseDown={(event) => event.stopPropagation()}>
        ...
        <div>
          <table>
            <tbody>
              {detail.rows.map(([key, value]) => (<tr key={key}>
                  <th>{key}</th>
                  <td>{value}</td>
                </tr>))}
              {!detail.rows.length && (<tr>
...
function ScanJobModal({ dialog, busy, targetCount, onClose, onSubmit, }: {
