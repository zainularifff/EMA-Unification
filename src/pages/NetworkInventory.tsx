...
function buildBreakdownVisual(rows: DrillRow[], area?: string, lens?: ReturnType<typeof getBreakdownLens>): BreakdownVisualModel | null {
  ...
  const titles: Record<string, { title: string; barTitle: string; headline: string; guidance: string }> = {
    ...
    saving: {
      title: "Optimization opportunity mix",
      barTitle: "Optimization opportunity ranking",
      headline: "Savings are split into cash, avoided spend and hidden productivity recovery.",
      guidance: "The chart helps avoid a shallow saving number by showing reuse, deferment, recovery and cleanup as separate opportunities.",
    },
    compliance: {
...
function getBreakdownLens(area?: string, title?: string) {
  ...
  const copy: Record<string, { label: string; title: string; description: string; valueLabel: string; recordLabel: string; evidenceLabel: string }> = {
    ...
    saving: {
      ...
      title: "Savings are shown as reuse, deferral, cleanup and productivity opportunities.",
      description: "This avoids treating cost saving as one number only. Each card explains whether the opportunity is tangible cash, avoided spend, or intangible operational recovery.",
      valueLabel: "Optimization value",
...
function getRowLens(row: DrillRow, area?: string, maxValue = 1, totalCount = 0) {
  ...
  const base = {
    ...
    costType: row.costType || (rowValue > 0 ? "Tangible" : "Intangible"),
    ...
    metricLabel: row.metricLabel || (rowValue > 0 ? "Estimated exposure" : "Evidence records"),
  };
      ...
      <section className="md-view-panel">
        <div className="md-view-header">
          <div>
            ...
            <p>{Number(drill.total || rows.length || 0).toLocaleString()} item(s). Each card explains value, risk type, confidence and next decision.</p>
          </div>
        ...
        <div className="md-view-body">
          {drill.loading ? <div className="md-state-panel">Loading breakdown...</div> : rows.length === 0 ? <div className="md-state-panel">No breakdown item is available for this selection.</div> : (
            <div className="md-executive-lens">
              <div className="md-lens-summary">
                <article className="md-lens-hero">
                  <span className="md-lens-label">{lens.label}</span>
                  <h3 className="md-lens-title">{lens.title}</h3>
                  <p className="md-lens-copy">{lens.description}</p>
                </article>
                <article className="md-lens-metric is-money">
                  <span>{lens.valueLabel}</span>
                  <strong>{totalValue > 0 ? formatMoney(totalValue) : `${Number(drill.total || totalCount || rows.length || 0).toLocaleString()}`}</strong>
                  <small>{totalValue > 0 ? "Tangible / estimated business exposure from this breakdown." : "No priced exposure found; use evidence records for follow-up."}</small>
                </article>
                <article className="md-lens-metric is-risk">
                  <span>{lens.recordLabel}</span>
                  <strong>{Number(totalCount || drill.total || rows.length || 0).toLocaleString()}</strong>
                  <small>Records represented by this management lens.</small>
                </article>
                <article className="md-lens-metric is-evidence">
                  <span>{lens.evidenceLabel}</span>
                  <strong>{confidence}%</strong>
                  <small>{costedRows.toLocaleString()} of {rows.length.toLocaleString()} category row(s) have costed value.</small>
                </article>
              </div>
              ...
              {visual && (
                <div className={`md-visual-panel is-${visual.type}`}>
                  <article className="md-visual-chart-card">
                    <div className="md-visual-head">
                      <div>
                        <span>{visual.modeLabel}</span>
                        <h3>{visual.title}</h3>
                        <p>{visual.description}</p>
                      </div>
                      <span className="md-visual-mode">Auto</span>
                    </div>

                    {visual.type === "donut" ? (
                      <div className="md-visual-donut-row">
                        <div className="md-visual-donut" style={{ "--donut": visual.gradient } as React.CSSProperties} aria-label={visual.title}>
                          <span className="md-visual-donut-center">
                            <strong>{visual.totalLabel}</strong>
                            <small>{visual.totalCaption}</small>
                          </span>
                        </div>
                        <div className="md-visual-side-stats">
                          {visual.items.slice(0, 4).map((item) => (
                            <button
                              type="button"
                              key={`visual-stat-${item.row.key || item.label}`}
                              className="md-visual-stat"
                            ...
                            >
                              <i className="md-visual-dot" />
                              <span>{item.shortLabel}<em>{item.display}</em></span>
                      ...
                      <div className="md-visual-bars">
                        {visual.items.map((item) => (
                          <button
                            type="button"
                            key={`visual-bar-${item.row.key || item.label}`}
                            className="md-visual-bar"
                            style={{ "--dot": item.color } as React.CSSProperties}
                            onClick={() => openLevel3(item.row.level3Area || drill.area || "risk", item.row.level3Key || item.row.key, item.label)}
                          >
                            <span className="md-visual-bar-top"><span>{item.shortLabel}</span><em>{item.display}</em></span>
                            <span className="md-visual-track"><i style={{ "--w": `${item.percent}%` } as React.CSSProperties} /></span>
                          </button>
                        ))}
                      </div>
                    )}
                  ...
                  <article className="md-visual-legend-card">
                    <span className="md-card-kicker">Distribution logic</span>
                    <h3>{visual.headline}</h3>
                    <p>{visual.guidance}</p>
                    <div className="md-visual-legend">
                      {visual.items.map((item) => (
                        <button
                          type="button"
                          key={`visual-legend-${item.row.key || item.label}`}
                          className="md-visual-legend-row"
                          style={{ "--dot": item.color } as React.CSSProperties}
                          onClick={() => openLevel3(item.row.level3Area || drill.area || "risk", item.row.level3Key || item.row.key, item.label)}
                        >
                          <i className="md-visual-dot" />
                          <span>{item.label}<em>{item.display}</em></span>
                          <strong>{item.percent}%</strong>
                        </button>
                      ))}
                    </div>
              ...
              <div className="md-decision-grid">
                {rows.map((row) => {
                  ...
                  return (
                    <button
                      ...
                      className={`md-decision-card tone-${normalizeTone(rowLens.tone)}`}
                      onClick={() => openLevel3(row.level3Area || drill.area || "risk", row.level3Key || row.key, row.label)}
                    >
                      <span className="md-card-kicker">{rowLens.impactType}</span>
                      <h3>{row.label}</h3>
                      <div>
                        <div className="md-decision-value-row">
                          <strong>{primary}</strong>
                          <span>{rowLens.metricLabel}<br />{countLabel}</span>
                        </div>
                        <div className="md-decision-progress" aria-hidden="true"><i style={{ "--w": `${rowLens.progress}%` } as React.CSSProperties} /></div>
                      </div>
                      <div>
                        <div className="md-decision-meta">
                          <span>{rowLens.costType}</span>
                          <span>{rowLens.riskType}</span>
                          <span>{rowLens.confidence}</span>
                        </div>
                        <p className="md-decision-insight">{rowLens.insight}</p>
                      </div>
                      <span className="md-decision-next"><span>{rowLens.decision}</span><em>Evidence <Icon name="next" /></em></span>
                    </button>
                  );
                })}
              </div>
