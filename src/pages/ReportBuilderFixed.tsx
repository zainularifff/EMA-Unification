...
function ensureOriginalPayload(payload: any, pack: Pack, range: Range) {
  ...
  return {
    ...
    report: {
      ...(data.report || {}),
      id: data.report?.id || pack.id,
      title: data.report?.title || pack.title,
      category: data.report?.category || pack.category,
      type: data.report?.type || pack.category,
      description: data.report?.description || pack.subtitle,
    },
    generatedAt: data.generatedAt || new Date().toISOString(),
    dateRange: data.dateRange || { from: range.from, to: range.to, preset: range.preset },
    narrative: {
      ...narrative,
      title: narrative.title || pack.title,
      period: narrative.period || `${range.from} to ${range.to}`,
      scope: narrative.scope || "All Sites",
...
function composePayload(title: string, range: Range, packs: Pack[], responses: any[]) {
  ...
  responses.forEach((response, index) => {
    ...
    actions.forEach((row: any) => recommendations.push({ ...row, area: row.area || pack.title }));
  });

  const executiveSummary = packs.map((pack, index) => `${index + 1}. ${pack.title}: ${summaryOf(responses[index], pack)}`).join("\n\n");
  return {
    report: { id: "ai-executive-summary", title, category: "Management Report Pack", type: "Dynamic Report Builder", description: "Combined report generated from the selected report canvas packs." },
    generatedAt: new Date().toISOString(),
    dateRange: { from: range.from, to: range.to, preset: range.preset },
    narrative: { title, period: `${range.from} to ${range.to}`, scope: "All Sites", executiveSummary, managementConclusion: executiveSummary, keyFindings: keyFindings.slice(0, 18) },
    metrics,
    sections,
...
export default function ReportBuilderFixed() {
  ...
  const runPreview = async () => {
    ...
    try { const payload = await buildReport("preview", title, customer, range, selected); const html = buildRegeneratedReportHtml(payload, filters(range, customer)); setPreview({ title: selected.length === 1 ? selected[0].title : title, html, count: selected.length }); }
