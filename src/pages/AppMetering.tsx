...
import { AlertCircle, AlertTriangle, CheckCircle, CheckCircle2, ChevronDown, ChevronLeft, ChevronRight, ChevronsLeft, ChevronsRight, Database, Download, Filter, Folder, FolderOpen, Gauge, Layers3, Package, Play, RefreshCw, Search, StopCircle, UserRound, X, } from "lucide-react";
...
function getDataArray<T>(payload: unknown): T[] {
    ...
    if (Array.isArray(dataObject?.rows))
        return dataObject.rows as T[];
    if (Array.isArray(envelope.raw))
        return envelope.raw as T[];
    if (Array.isArray(dataObject?.raw))
        return dataObject.raw as T[];
    return [];
}
...
function getStatus(application: string, launchCount: number, usedTimeHours: number, usedTimeSeconds = 0): UsageStatus {
    ...
    if (restrictedKeywords.some((keyword) => app.includes(keyword)))
        return "Restricted";
    if (launchCount <= 0 && usedTimeSeconds <= 0)
        return "Unused";
    if (usedTimeSeconds <= 0)
        return "Unused";
    if (usedTimeHours < 2 || launchCount < 5)
        return "Review";
    return "Productive";
}
function getRisk(status: UsageStatus): RiskLevel {
    if (status === "Restricted")
        return "High";
    if (status === "Review" || status === "Unused")
        return "Medium";
    return "Low";
}
function getLicenseType(application: string): LicenseType {
    const app = application.toLowerCase();
    if (enterpriseKeywords.some((keyword) => app.includes(keyword)))
        return "Enterprise";
    if (paidKeywords.some((keyword) => app.includes(keyword)))
        return "Paid";
    if (["chrome", "firefox", "edge"].some((keyword) => app.includes(keyword)))
        return "Free";
    return "Unknown";
}
function getRecommendation(status: UsageStatus, risk: RiskLevel, licenseType: LicenseType) {
    if (risk === "High")
        return "Validate business need immediately or move this application into policy restriction review.";
    if (status === "Unused" && licenseType === "Paid")
        return "Candidate for license reclaim because no usage was detected in the selected window.";
    if (status === "Review")
        return "Review usage pattern before renewal or standardization decision.";
...
function collectFolderRelationIds(node: TreeNode | null): Set<number> {
    ...
    const walk = (item: TreeNode | null | undefined) => {
        if (!item)
            return;
        if (item.type === "folder" && typeof item.relationID === "number" && item.relationID > 0)
            ids.add(item.relationID);
...
function collectRelationIds(nodes: TreeNode[], ids = new Set<number>()): Set<number> {
    for (const node of nodes) {
        if (node.type === "folder" && typeof node.relationID === "number")
            ids.add(node.relationID);
        if (node.children?.length)
            collectRelationIds(node.children, ids);
    }
    return ids;
}
...
function getTreeStatusPillClass(status = "") {
    ...
    if (treeStatus === "online")
        return "user-pill active";
    if (treeStatus === "offline")
        return "user-pill hardware-status-pill is-offline appm-status-offline";
    return "user-pill muted-cell";
}
function getUsageStatusPillClass(status: UsageStatus) {
    const value = statusClass(status);
    return cx("user-pill", value === "productive" && "active", value === "restricted" && "locked", value === "review" && "review", value === "unused" && "muted-cell");
}
function getRiskPillClass(risk: RiskLevel) {
    const value = riskClass(risk);
    return cx("user-pill", value === "low" && "active", value === "medium" && "review", value === "high" && "locked");
}
...
function getAppMeteringTreeCount(node: TreeNode): number {
    if (typeof node.count === "number" && Number.isFinite(node.count) && node.count > 0)
        return node.count;
    if (node.type === "device" || node.type === "package")
        return 1;
    return (node.children || []).reduce((total, child) => total + getAppMeteringTreeCount(child), 0);
}
function AppMeteringTree({ nodes, selectedId, onSelect, search = "", }: {
    nodes: TreeNode[];
    selectedId: string;
    ...
    const renderNode = (node: TreeNode, depth = 0): ReactNode => {
        if (!appMeteringTreeMatchesSearch(node, keyword))
            return null;
        ...
        <div>
          <button type="button" aria-label={hasChildren ? (isOpen ? `Collapse ${node.label}` : `Expand ${node.label}`) : node.label} onClick={(event) => {
          ...
          <button type="button" title={node.subLabel ? `${node.label} · ${node.subLabel}` : node.label} onClick={handleSelect}>
...
export default function AppMetering() {
    ...
    const currentMeteringScopeNode = useMemo(() => {
        if (selectedNode.type === "device")
            return selectedNode;
        if (selectedNode.id === "organization")
    ...
    const activeFilters = useMemo(() => {
        ...
        if (oneYearMode)
            params.set("oneYear", "true");
        if (nextPageMode)
            params.set("nextpage", "true");
        return params;
    ...
    useEffect(() => {
        if (!oneYearMode && nextPageMode)
            setNextPageMode(false);
    ...
    const handleTreeSearch = (value: string) => {
        ...
        if (value.trim() && match) {
            ...
            if (match.type === "package")
                setSelectedPackageId(match.packageId || 0);
        }
    };
    ...
    return (<main data-section="application-metering">
          ...
          <div>
            <div>
              ...
              <div role="tree" aria-label="Application metering target tree">
                ...
                {loading.hierarchy || loading.packages ? (<div>{viewMode === "device" ? "Preparing branch view..." : "Preparing package list..."}</div>) : activeTree.length > 0 ? (<AppMeteringTree nodes={activeTree} selectedId={selectedNode.id} onSelect={handleNodeSelect} search={treeSearch}/>) : (<div>{viewMode === "device" ? "No branch entries found." : "No packages found."}</div>)}
              </div>
            </div>
          </div>
        </aside>
            ...
            <div>
              <div>
                ...
                <div>
                  ...
                  <button type="button" onClick={() => handleScopeMeteringToggle(currentMeteringScopeNode)} disabled={loading.action} title={currentMeteringButtonTitle}>
...
<option value="nextpage">Next Page</option>
              ...
              <div>
                {showDeviceRegistry ? (<table>
                    ...
                    <tbody>
                      {loading.assets ? (<tr><td colSpan={8}><div><strong>Loading devices</strong><span>Loading devices from {selectedNode.relationID}...</span></div></td></tr>) : pagedDeviceRows.length === 0 ? (<tr><td colSpan={8}><div><strong>No devices found</strong><span>{selectedNode.id === "organization" ? "Company scope selected. Choose a department to browse devices, or run Metering Company directly." : "No devices found in this folder scope."}</span></div></td></tr>) : pagedDeviceRows.map((device, index) => {
                const raw = device.raw || {};
                const isSelected = selectedNode.id === device.id;
                return (<tr key={device.id} onClick={() => handleNodeSelect(device)}>
                    ...
                    <tbody>
                      {loading.usage ? (<tr><td colSpan={9}><div><strong>Loading usage records</strong><span>Please wait while the application metering registry is refreshed.</span></div></td></tr>) : pagedRows.length === 0 ? (<tr><td colSpan={9}><div><strong>No records found</strong><span>No application metering records found for current filter.</span></div></td></tr>) : pagedRows.map((row) => (<tr key={`${row.id}-${row.application}-${row.device}`} onClick={() => setSelectedRowId(row.id)}>
      ...
      {drawerRow ? (<div onClick={() => setDrawerRow(null)}>
          <section onClick={(event) => event.stopPropagation()}>
            ...
            <div>
              ...
              <div>
                {packageFiles.length === 0 ? (<><strong>No package file group loaded</strong><span>Choose a package filter to call for a package file group.</span></>) : (<div>
                    {packageFiles.slice(0, 8).map((file) => (<div key={`${file.id}-${file.fileName}`}>
      ...
      {showMeteringModal ? (<div onClick={() => setShowMeteringModal(false)}>
          <section onClick={(event) => event.stopPropagation()}>
            <div>
              <div>
