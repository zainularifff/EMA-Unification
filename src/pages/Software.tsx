...
import { AlertTriangle, BarChart3, ChevronDown, ChevronRight, Database, Download, FileText, Folder, FolderOpen, Layers, MonitorSmartphone, Package, RefreshCw, Search, ShieldCheck, X, } from "lucide-react";
...
function buildStatTree(summary: {
    totalRecords: number;
    categories: number;
...
function statEndpoint(stat: StatView) {
    if (stat === "installed")
        return "/api/software/statistics/installed-software";
    if (stat === "license")
        return "/api/software/statistics/license-status";
    if (stat === "exe")
        return "/api/software/statistics/exe-files";
    if (stat === "dll")
        return "/api/software/statistics/dll-files";
...
function statTitle(stat: StatView | null) {
    ...
    if (stat === "exe")
        return "EXE File Extension";
    if (stat === "dll")
        return "DLL File Extension";
    if (stat === "ini")
        return "INI File Extension";
    return "";
}
function Software() {
    ...
    const fetchSoftwareRecords = async () => {
        ...
        setError("");
        try {
            const payload = await apiGet<ApiSoftwareRecord[] | {
                data?: ApiSoftwareRecord[];
    ...
    const fetchDepartmentTree = async () => {
        ...
        setTreeError("");
        try {
            const payload = await apiGet<DepartmentNode[] | {
                data?: DepartmentNode[];
    ...
    const filteredRecords = useMemo(() => {
        ...
        if (activeView === "categories")
            next = next.filter((record) => record.category.toLowerCase() !== "unclassified");
        if (activeView === "unclassified")
            next = next.filter((record) => record.category.toLowerCase() === "unclassified");
        if (categoryFilter !== "all")
            next = next.filter((record) => record.category === categoryFilter);
        if (typeFilter !== "all")
            next = next.filter((record) => record.machineType === typeFilter);
        if (term) {
    ...
    const refreshCurrentView = async () => {
        if (selectedStat)
            await loadStatRows(selectedStat);
        else
            await fetchSoftwareRecords();
    };
    ...
    const renderTree = (nodes: TreeNode[], depth = 0, mode: SidebarTab = "branch") => (<div>
      {nodes.map((node) => {
    ...
    return (<main data-section="software">
      {toast && (<section>
      ...
      <section>
        ...
        <section>
        ...
        ].map((card) => (<button key={card.key} type="button" onClick={() => activateView(card.key)}>
          ...
          <section>
            ...
            {selectedStat ? (<section>
                ...
                {!statLoading && !statError && currentStatRows.length > 0 && (<table>
                    <thead>
                      <tr>{["No", "Name", "Version", "Publisher", "Reference", "Count"].map((header) => <th key={header}>{header}</th>)}</tr>
                    </thead>
                    <tbody>
                      {currentStatRows.map((row, index) => (<tr key={`${selectedStat}-${index}`}>
                ...
                {!loading && !error && currentRows.length > 0 && (<table>
                    <thead>
                      <tr>
                        {[
                    ...
                    <tbody>
                      {currentRows.map((record) => (<tr key={record.id}>
