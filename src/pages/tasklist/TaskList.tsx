...
function TaskTargetList({
  ...
  if (sidePanel) {
    return (
      <section className="task-card task-target-card task-target-side-card">
        <div className="task-panel-head task-panel-head-between">
          <div className="task-panel-title">
            ...
            <div>
              ...
              <p>Assigned targets and individual endpoint status.</p>
            </div>
          </div>
          <span className="task-badge-outline">Targets {targets.length}</span>
        </div>

        <div className="task-target-side-body">
          {isBusy ? (
            <div className="task-empty-state compact"><Loader2 size={20} className="task-spin" /> Loading target status...</div>
          ) : task && displayRows.length ? displayRows.map((row, index) => (
            <article className="task-target-side-item" key={row.key}>
              <div className="task-target-side-top">
                <span className="task-target-index">{String(index + 1).padStart(2, "0")}</span>
                <span className={`task-status-pill ${getStateClass(row.status)}`}>{row.status}</span>
              </div>
              <strong className="task-target-name">{row.name}</strong>
              <div className="task-target-side-meta">
                ...
                <span>Type</span>
                <b>{row.type}</b>
                <span>{hasExecutionRows ? "Last Changed" : "Last Connection"}</span>
                <b>{row.lastActivity}</b>
                <span>Device ID</span>
                <b>{row.deviceId}</b>
              </div>
            </article>
        ...
        <div className="task-combined-table-foot task-target-side-foot">
          <span>Showing {totalRows} endpoint row{totalRows === 1 ? "" : "s"}.</span>
        </div>
      </section>
    );
  }
