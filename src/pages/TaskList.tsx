...
import { AlertCircle, CheckCircle2, ClipboardList, Download, Loader2, RefreshCw, Search, Square, Trash2, UserRound, X, } from "lucide-react";
import taskListService from "../services/taskListService";
type AppButtonVariant = "primary" | "secondary" | "success" | "danger" | "warning" | "info" | "light" | "dark" | "outline-primary" | "outline-secondary" | "outline-success" | "outline-danger" | "outline-warning" | "outline-info" | "outline-light" | "outline-dark";
...
function getGlobalButtonClass(variant: AppButtonVariant) {
    if (variant === "danger" || variant === "outline-danger")
        return "danger-btn";
    if (variant === "primary" || variant === "success" || variant === "info")
        return "primary-btn";
    return "soft-btn";
}
function AppButton({ variant = "primary", loading = false, leftIcon, disabled, children, type = "button", ...props }: AppButtonProps) {
    return (<button type={type} disabled={disabled || loading} {...props}>
...
function AppPagination({ currentPage, totalPages, totalItems = 0, pageSize = 10, showInfo = false, disabled = false, onPageChange, }: AppPaginationProps) {
...
type SortKey = keyof Pick<TaskItem, "id" | "classification" | "taskType" | "commandType" | "state" | "description" | "startTime" | "endTime" | "scheduledTime" | "orderedBy">;
type TaskItem = {
    id: number;
    jobId?: number;
    jobType?: number;
...
type TaskProgress = Pick<TaskItem, "classification" | "startTime" | "transferRate" | "completionRate" | "totalObjects" | "commandCompleted" | "taskCompleted" | "taskRunning" | "commandIncomplete" | "taskEnd"> & {
    jobId?: number;
    raw?: Record<string, unknown>;
};
...
function normalizeTask(input: Partial<TaskItem> & Record<string, unknown>): TaskItem {
    ...
    const actionDisabledReason = normalizeString(input.actionDisabledReason ?? input.ActionDisabledReason, isTerminal ? "Action disabled because this task is already completed, stopped, or cancelled." : "");
    return {
...
const TaskList = () => {
    ...
    useEffect(() => {
        document.documentElement.classList.add("ema-layout-lock", "ema-settings-page-active");
        document.body.classList.add("ema-layout-lock", "ema-settings-page-active");
        return () => {
            document.documentElement.classList.remove("ema-layout-lock", "ema-settings-page-active");
            document.body.classList.remove("ema-layout-lock", "ema-settings-page-active");
        };
    }, []);
    ...
    const sortButton = (label: string, key: SortKey) => (<button type="button" onClick={() => handleSort(key)}>
      <span>{label}</span>
      <i aria-hidden="true">{renderSortIndicator(key)}</i>
    </button>);
    return (<>
      <AppToast show={Boolean(toast)} tone={toast?.type || "info"} title={toast?.title} message={toast?.message || ""} onClose={() => setToast(null)}/>
          ...
          <section>
            ...
            <section>
              <div>
                <label>
                  <Search size={15}/>
                  <input value={searchTerm} onChange={(event) => setSearchTerm(event.target.value)} placeholder="Search job ID, command, task, state or ordered by..."/>
                  {searchTerm ? (<button type="button" onClick={() => setSearchTerm("")} aria-label="Clear search">
                      <X size={14}/>
                    </button>) : null}
                </label>
              ...
              <div>
                ...
                <div>
                  ...
                  {!isLoading && paginatedTasks.map((task, index) => (<div key={task.id} onClick={() => openTaskDetails(task.id)}>
                      <div>
              ...
              <progress className="task-progress-native" value={Math.min(100, Math.max(0, task.completionRate))} max={100} />
                      ...
                      <div>
                        <div onClick={(event) => event.stopPropagation()}>
                          <button type="button" title={getTaskActionDisabledTitle("stop", task)} aria-label={getTaskActionDisabledTitle("stop", task)} disabled={!canRunTaskAction("stop", task)} onClick={() => requestTaskAction("stop", task)}>
                            <Square size={14}/>
                          </button>
                          <button type="button" title={getTaskActionDisabledTitle("cancel", task)} aria-label={getTaskActionDisabledTitle("cancel", task)} disabled={!canRunTaskAction("cancel", task)} onClick={() => requestTaskAction("cancel", task)}>
                            <X size={14}/>
                          </button>
                          <button type="button" title={getTaskActionDisabledTitle("delete", task)} aria-label={getTaskActionDisabledTitle("delete", task)} disabled={!canRunTaskAction("delete", task)} onClick={() => requestTaskAction("delete", task)}>
                            <Trash2 size={14}/>
                          </button>
                        </div>
                      </div>
                ...
                {!isLoading && filteredTasks.length > 0 && (<AppPagination currentPage={clampPage(currentPage, totalPages)} totalPages={totalPages} totalItems={filteredTasks.length} pageSize={pageSize} disabled={isLoading} onPageChange={(page) => setCurrentPage(clampPage(page, totalPages))}/>)}
              </div>
            </section>
          </section>
        </div>
      ...
      {isDetailOpen && selectedTask && (<TaskDetailModal task={selectedTask} progress={selectedProgress} targets={targets} progressDetails={progressDetails} stateOptions={stateOptions} isLoading={isDetailLoading} isProgressLoading={isProgressLoading} onClose={closeTaskDetails} onRefresh={() => selectedTaskId && loadTaskDetail(selectedTaskId)} onAction={requestTaskAction}/>)}

      {pendingAction && (<TaskActionModal pendingAction={pendingAction} isLoading={isActionLoading} onCancel={() => setPendingAction(null)} onConfirm={confirmTaskAction}/>)}
    </>);
};
function TaskProcedureStatus({ task, progress }: {
    task: TaskItem | null;
...
function TaskTargetList({ task, targets, progressDetails, stateOptions, isLoading, isProgressLoading, compactHeader = false, sidePanel = false, }: {
    task: TaskItem | null;
    targets: TargetItem[];
    progressDetails: ProgressDetailItem[];
    ...
    const targetLookup = useMemo(() => {
        ...
        targets.forEach((target) => {
            if (target.objectRootIdn)
                lookup.set(String(target.objectRootIdn), target);
            if (target.username)
                lookup.set(target.username.toLowerCase(), target);
            if (target.objectDeviceID)
                lookup.set(target.objectDeviceID.toLowerCase(), target);
        });
        return lookup;
    }, [targets]);
    const displayRows = useMemo(() => {
        const rows = hasExecutionRows
            ? progressDetails.map((row, index) => {
                ...
                const matchedTarget = targetLookup.get(objectRoot) ||
    ...
    const filteredTargetRows = useMemo(() => {
        ...
        if (!query)
            return displayRows;
        return displayRows.filter((row) => [row.name, row.department, row.ipAddress, row.type, row.status, row.lastActivity, row.deviceId]
    ...
    if (sidePanel) {
        ...
        <div aria-label="Target endpoint filters">
          <label>
            <Search size={15}/>
            <input type="search" value={targetSearch} onChange={(event) => setTargetSearch(event.target.value)} placeholder="Search target, IP, status, device ID..." aria-label="Search target endpoints"/>
          </label>
          {targetSearch ? (<button type="button" onClick={() => setTargetSearch("")}>Clear</button>) : null}
        </div>

        <div role="list" aria-label="Target endpoint compact list">
          {isBusy ? (<div><Loader2 size={20}/> Loading target status...</div>) : task && visibleRows.length ? visibleRows.map((row, index) => (<article key={row.key} role="listitem">
            ...
            </article>)) : (<div>{targetSearch ? "No target matches your search." : emptyMessage}</div>)}
        </div>
        ...
        <table>
          ...
          <tbody>
            {isBusy ? (<tr>
              ...
              </tr>) : task && displayRows.length ? displayRows.map((row) => (<tr key={row.key}>
...
function TaskDetailModal({ task, progress, targets, progressDetails, stateOptions, isLoading, isProgressLoading, onClose, onRefresh, onAction, }: {
    task: TaskItem;
    progress: TaskProgress | null;
    targets: TargetItem[];
    progressDetails: ProgressDetailItem[];
    ...
    return (<div onClick={onClose} role="presentation">
      <section role="dialog" aria-modal="true" aria-label={`Task ${task.id} detail`} onClick={(event) => event.stopPropagation()}>
        ...
        <div>
          ...
          <TaskTargetList task={task} targets={targets} progressDetails={progressDetails} stateOptions={stateOptions} isLoading={isLoading} isProgressLoading={isProgressLoading} sidePanel/>
        </div>
      </section>
    </div>);
}
function TaskRightPanel({ task, onAction, }: {
    task: TaskItem | null;
    onAction: (action: TaskAction, task: TaskItem) => void;
}) {
    if (!task) {
...
function TaskActionModal({ pendingAction, isLoading, onCancel, onConfirm, }: {
    ...
    return (<AppModal show size="sm" onHide={onCancel} eyebrow="Confirmation" title={<span>
