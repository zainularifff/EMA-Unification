...
export default function ReportBuilderRules() {
  ...
  const selectedBranch = useMemo(() => branchOptions.find((branch) => branch.id === branchId) || branchOptions[0] || FALLBACK_BRANCHES[0], [branchId, branchOptions]);
  ...
  return (
    <main className="builder-page">
      ...
      <section className="builder-top">
        ...
        <div className="actions"><button className="btn" onClick={runPreview} disabled={Boolean(loading)}>{loading === "preview" ? "Loading..." : "Preview"}</button><button className="btn primary" onClick={runGenerate} disabled={Boolean(loading)}>{loading === "generate" ? "Generating..." : "Generate Report"}</button></div>
      </section>
      ...
      <section className="builder-layout">
        ...
        <aside className="panel right"><div className="head"><strong>Build Summary</strong><small>{selected.length} selected report pack{selected.length === 1 ? "" : "s"}</small></div><div className="summary"><div className="metric"><span>Branch Scope</span><strong>{branchLabel(selectedBranch)}</strong></div><div className="metric"><span>Period</span><strong>{range.from} → {range.to}</strong></div><div className="metric"><span>Output</span><strong>{hasSummary ? "Standalone PDF" : selected.length > 1 ? "Combined PDF" : "Legacy PDF Design"}</strong></div><div className="rule-note">AI Executive Summary is standalone. Client RNR, Hardware Lifecycle, Security, Software, Ops and AI Dynamic Reporting can be combined.</div>{selected.length ? selected.map((pack, index) => <div className="selected-pill" key={`${pack.id}-${index}`}><span className="ico" style={{ "--accent": pack.tone } as CSSProperties}>{pack.icon}</span>{index + 1}. {pack.title}</div>) : <div className="metric"><span>Status</span><strong>Canvas empty</strong></div>}</div></aside>
      </section>
      {preview && <div className="backdrop" onClick={(event) => event.target === event.currentTarget && setPreview(null)}><section className="preview"><div className="preview-head"><div><strong>{preview.title}</strong><small>{range.from} to {range.to} · {preview.count} pack{preview.count === 1 ? "" : "s"} · {branchLabel(selectedBranch)}</small></div><button className="btn" onClick={() => setPreview(null)}>Close</button></div><iframe className="preview-frame" title={`${preview.title} preview`} srcDoc={preview.html} /></section></div>}
    </main>
  );
}
