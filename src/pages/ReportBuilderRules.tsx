...
import { buildLegacyReportHtml } from "../utils/reportPdfLegacyDesign";
...
function rangeOf(preset: Period): Range {
  ...
  if (preset === "previous-month") { const prev = new Date(now.getFullYear(), now.getMonth() - 1, 1); return { preset, from: monthStart(prev), to: monthEnd(prev) }; }
  if (preset === "this-quarter") { const q = Math.floor(now.getMonth() / 3) * 3; return { preset, from: iso(new Date(now.getFullYear(), q, 1)), to: iso(now) }; }
...
function requestPayload(title: string, customer: string, range: Range, pack: Pack) {
  return { ...filters(range, customer), reportId: pack.id, dynamicReportType: pack.dynamic ? pack.id : undefined, dynamicReportTitle: pack.dynamic ? pack.title : undefined, customReportTitle: pack.title, parentReportTitle: title };
}
function ensureOriginalPayload(payload: any, pack: Pack, range: Range) {
  ...
  return { ...data, report: { ...(data.report || {}), id: data.report?.id || pack.id, title: data.report?.title || pack.title, category: data.report?.category || pack.category, type: data.report?.type || pack.category, description: data.report?.description || pack.subtitle }, generatedAt: data.generatedAt || new Date().toISOString(), dateRange: data.dateRange || { from: range.from, to: range.to, preset: range.preset }, narrative: { ...narrative, title: narrative.title || pack.title, period: narrative.period || `${range.from} to ${range.to}`, scope: narrative.scope || "All Sites", executiveSummary: summary, managementConclusion: narrative.managementConclusion || summary, keyFindings: Array.isArray(narrative.keyFindings) && narrative.keyFindings.length ? narrative.keyFindings : [summary] }, sections: Array.isArray(data.sections) ? data.sections : [], recommendations: Array.isArray(data.recommendations) ? data.recommendations : [], metrics: data.metrics || {}, filters: { ...(data.filters || {}), startDate: range.from, endDate: range.to, dateRange: range.preset } };
}
...
function composePayload(title: string, range: Range, packs: Pack[], responses: any[]) {
  if (packs.length === 1) return ensureOriginalPayload(responses[0], packs[0], range);
  const sections: any[] = [];
  ...
  return { report: { id: "report-pack-builder", title, category: "Management Report Pack", type: "Dynamic Report Builder", description: "Combined report generated from selected canvas report packs." }, generatedAt: new Date().toISOString(), dateRange: { from: range.from, to: range.to, preset: range.preset }, narrative: { title, period: `${range.from} to ${range.to}`, scope: "All Sites", executiveSummary, managementConclusion: executiveSummary, keyFindings: keyFindings.slice(0, 18) }, metrics, sections, recommendations: recommendations.length ? recommendations : packs.map((pack) => ({ priority: "Review", action: `Review ${pack.title} findings and assign owner.`, owner: "Management Team" })), builderPacks: packs.map((pack, index) => ({ order: index + 1, id: pack.id, title: pack.title, category: pack.category })) };
}
...
function downloadHtml(title: string, payload: any, range: Range, customer: string) {
  const html = buildLegacyReportHtml(payload, filters(range, customer));
...
export default function ReportBuilderRules() {
  ...
  const addPack = (pack: Pack, target?: number) => setSlots((current) => {
    if (isSummary(pack)) {
      ...
      next[0] = pack;
      setError("AI Executive Summary is a standalone report and cannot be combined with other report packs.");
      return next;
    }
  ...
  const runPreview = async () => {
    ...
    try { const payload = await buildReport("preview", title, customer, range, selected); const html = buildLegacyReportHtml(payload, filters(range, customer), { preview: true, autoPrint: false }); setPreview({ title: selected.length === 1 ? selected[0].title : title, html, count: selected.length }); }
  ...
  const runGenerate = async () => {
    ...
    try { const payload = await buildReport("generate", title, customer, range, selected); downloadHtml(selected.length === 1 ? selected[0].title : title, payload, range, customer); }
  ...
  return (
    <main className="builder-page">
      ...
      <section className="builder-top">
        ...
        <label className="field"><span>Date Range</span><select value={range.preset} onChange={(e) => setRange(rangeOf(e.target.value as Period))}>{PERIODS.map((item) => <option key={item.value} value={item.value}>{item.label}</option>)}</select></label>
