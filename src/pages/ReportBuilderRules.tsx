...
function ensureOriginalPayload(payload: any, pack: Pack, range: Range) {
  ...
  return {
    ...
    filters: { ...(data.filters || {}), startDate: range.from, endDate: range.to, dateRange: range.preset },
  };
}
...
function downloadHtml(title: string, payload: any, range: Range, customer: string) {
  const html = buildBuilderReportHtml(payload, filters(range, customer));
...
export default function ReportBuilderRules() {
  ...
  const runPreview = async () => {
    ...
    try { const payload = await buildReport("preview", title, customer, range, selected); const html = buildBuilderReportHtml(payload, filters(range, customer), { preview: true, autoPrint: false }); setPreview({ title: selected.length === 1 ? selected[0].title : title, html, count: selected.length }); }
