...
function defaultRange(): Range { const end = new Date(); const start = new Date(); start.setDate(end.getDate() - 29); return { preset: "last-30-days", from: iso(start), to: iso(end) }; }
...
function baseFilters(range: Range, pack?: Pack) {
  return { ...reportScope(), dateRange: range.preset, startDate: range.from, endDate: range.to, outputFormat: "PDF", reportId: pack?.id, includeSummary: true, includeChart: true, includeTable: true, includeRecommendation: true };
}
...
function combinePayload(title: string, range: Range, packs: Pack[], payloads: any[]) {
...
function openPrint(title: string, payload: any, range: Range) {
...
export default function ReportBuilderRulesClean() {
  ...
  const runReport = async (mode: "preview" | "generate") => {
    ...
    try {
      ...
      const payload = combinePayload(title, range, selected, payloads);
      if (mode === "generate") openPrint(selected.length === 1 ? selected[0].title : title, payload, range);
      else setPreview({ title: selected.length === 1 ? selected[0].title : title, html: buildBuilderReportHtml(payload, baseFilters(range), { preview: true, autoPrint: false }), count: selected.length });
    } finally { setLoading(""); }
  };
  ...
  return (
    <main className="builder-page">
      <section className="builder-top">
        ...
        <label className="field"><span>Date Range</span><select value={range.preset} onChange={(event) => setRange(defaultRange())}><option value="last-30-days">Last 30 Days</option></select></label>
