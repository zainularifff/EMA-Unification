...
import { Boxes, ChevronDown, ChevronLeft, ChevronRight, ChevronsLeft, ChevronsRight, CircleDot, Clock3, Cloud, Database, ExternalLink, FileDown, Folder, FolderOpen, FolderPlus, Globe2, Info, Laptop, ListChecks, Loader2, PackageCheck, Play, RefreshCw, Search, Server, ShieldAlert, ShieldCheck, X, } from 'lucide-react';
import { createOnlinePatchScanJob, getAssetsByRelationID, getDepartments, getOnlinePatchCatalog, getOnlinePatchDetail, getOnlinePatchStatus, getOnlinePatchSummary, prepareOnlinePatchInstall, type AssetItem, type DepartmentNode, type OnlinePatchDetail, type OnlinePatchQueryParams, type OnlinePatchRow, type OnlinePatchScopeParams, type OnlinePatchStatusFilter, type OnlinePatchSummary, } from '../services/patchService';
...
function filterDepartmentNodes(nodes: DepartmentNode[], keyword: string): DepartmentNode[] {
    ...
    return nodes
        ...
        if (label.includes(text) || children.length)
...
const getKbText = (row?: OnlinePatchRow | null) => {
    if (!row)
        return '-';
    if (row.KB)
        return String(row.KB);
    if (Array.isArray(row.KBArticleIDs) && row.KBArticleIDs.length)
        return row.KBArticleIDs.join(', ');
    return '-';
};
...
function PatchManagement() {
    ...
    const loadAssetsForRelation = useCallback(async (relationID: number) => {
        if (!relationID || assetsByRelation[relationID] || loadingAssets[relationID])
            return;
    ...
    const runConfirmedAction = async () => {
        ...
        try {
            ...
            if (confirmState.type === 'install') {
                ...
                if (!objectRootIdn)
                    throw new Error('Please select a device or use a device status row before installing a patch.');
    ...
    return (<main data-section="patch-management">
      ...
      <div>
        <aside>
          ...
          <nav role="tablist" aria-label="Patch navigation">
            ...
            <button type="button" onClick={() => setSidebarMode('statistics')}>
          ...
          <div>
            <div>
              {sidebarMode === 'organization' ? (<>
                  <div>
                    <Search size={15}/>
                    <input placeholder="Search branches..." value={scopeSearch} onChange={(event) => setScopeSearch(event.target.value)}/>
                  </div>

                  <button type="button" onClick={() => showToast({ type: 'info', message: 'Branch path creation is managed from the organization settings.' })}>
                    <FolderPlus size={13}/>
                    New Branch Path
                  </button>
                  ...
                  <div aria-label="Patch scope tree">
                    <div>
                      ...
                      <div>
                        {filteredDepartments.map((node) => (<TreeNode key={node.Object_Rel_Idn} node={node} level={1} selectedScope={selectedScope} expanded={expanded} assetsByRelation={assetsByRelation} loadingAssets={loadingAssets} onToggle={toggleDepartment} onSelectDepartment={selectDepartment} onSelectDevice={selectDevice}/>))}
                      </div>
                    </div>
        ...
        <section>
          ...
          <main>
            ...
            <div>
              ...
              {mode === 'offline' ? (<OfflinePlaceholder />) : loadingData ? (<div>
                  <Loader2 size={18}/> Loading patch data...
                </div>) : (<PatchTable rows={pagedRows} activeTab={activeTab} page={page} limit={limit} onOpenDetails={openDetails} onInstall={(row) => setConfirmState({ type: 'install', row })}/>)}
      ...
      {selectedPatch && (<PatchDetailDrawer row={selectedPatch} detail={patchDetail} loading={detailLoading} onClose={() => {
                setSelectedPatch(null);
                setPatchDetail(null);
            }} onInstall={(row) => setConfirmState({ type: 'install', row })}/>)}

      {confirmState && (<ConfirmModal state={confirmState} scope={selectedScope} loading={actionLoading} onCancel={() => setConfirmState(null)} onConfirm={runConfirmedAction}/>)}
    </main>);
}
function TreeNode({ node, level, selectedScope, expanded, assetsByRelation, loadingAssets, onToggle, onSelectDepartment, onSelectDevice, }: {
    node: DepartmentNode;
    level: number;
    ...
    return (<div>
      ...
      {isExpanded && (<div>
          {children.map((child) => (<TreeNode key={child.Object_Rel_Idn} node={child} level={level + 1} selectedScope={selectedScope} expanded={expanded} assetsByRelation={assetsByRelation} loadingAssets={loadingAssets} onToggle={onToggle} onSelectDepartment={onSelectDepartment} onSelectDevice={onSelectDevice}/>))}
          ...
          {devices.map((asset) => {
                ...
                return (<div key={`${relationID}-${deviceID}-${getDeviceName(asset)}`}>
                <div>
                  <button type="button" aria-label={getDeviceName(asset)}><span /></button>
                  <button type="button" onClick={() => onSelectDevice(asset, relationID)} title={getDeviceName(asset)}>
...
function KpiCard({ label, value, note, icon, color, attention, active, onClick, }: {
    label: string;
    value: ReactNode;
    note: ReactNode;
    icon: ReactNode;
    ...
    return (<button type="button" onClick={onClick} aria-pressed={Boolean(active)} title={`${label}: click to filter patch table`}>
      <div>
        <i>{icon}</i>
        <span>{label}</span>
        <strong>{value}</strong>
...
function PatchTable({ rows, activeTab, page, limit, onOpenDetails, onInstall }: {
    rows: OnlinePatchRow[];
    activeTab: PatchTab;
    page: number;
    limit: number;
    onOpenDetails: (row: OnlinePatchRow) => void;
      ...
      <table>
        ...
        <tbody>
          {rows.map((row, index) => {
            ...
            return (<tr key={getPatchRowKey(row, index)}>
                ...
                <td>
                  <div>
                    {canInstall && (<button type="button" onClick={() => onInstall(row)}>
...
function PatchDetailDrawer({ row, detail, loading, onClose, onInstall }: {
    ...
    return (<div>
      <section>
        ...
        <div>
          {loading ? (<div><Loader2 size={18}/> Loading patch detail...</div>) : (<>
              <section>
                <div>
                  <div>
              ...
              <section>
                ...
                <div>
                  {(detail?.files || []).length ? detail?.files.map((file) => (<div key={`${file.UpdateID}-${file.RevisionNumber}-${file.FileName}`}>
...
function PatchToast({ toast, onClose }: {
...
function ExternalRow({ label, url }: {
    label: string;
    url: string;
}) {
