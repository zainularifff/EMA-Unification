...
function AppPagination({
  ...
  return (
    <div className={`uam-pagination global-style task-settings-pagination ${className}`.trim()}>
      <div className="uam-page-summary">Page {safeCurrentPage} of {safeTotalPages}</div>
      {showInfo ? (
        <div className="uam-pagination-info">
          Showing <strong>{firstItem}-{lastItem}</strong> of <strong>{totalItems}</strong> task records
        </div>
      ) : null}
      <div className="uam-pagination-controls global-style" aria-label="Task list pagination">
        <button className="uam-page-icon" type="button" disabled={disabled || safeCurrentPage <= 1} onClick={() => onPageChange(1)} aria-label="First page">«</button>
        <button className="uam-page-icon" type="button" disabled={disabled || safeCurrentPage <= 1} onClick={() => onPageChange(safeCurrentPage - 1)} aria-label="Previous page">‹</button>
        <span className="uam-page-current">{safeCurrentPage}</span>
        <button className="uam-page-icon" type="button" disabled={disabled || safeCurrentPage >= safeTotalPages} onClick={() => onPageChange(safeCurrentPage + 1)} aria-label="Next page">›</button>
        <button className="uam-page-icon" type="button" disabled={disabled || safeCurrentPage >= safeTotalPages} onClick={() => onPageChange(safeTotalPages)} aria-label="Last page">»</button>
      </div>
    </div>
  );
}
...
const TaskList = () => {
  ...
  const taskColumns = useMemo<AppTableColumn<TaskItem>[]>(() => [
    {
      key: "rowNumber",
      header: "#",
      width: 62,
      render: (_task, index) => <span className="ema-row-no">{pageStart + index}</span>,
    },
    {
      key: "id",
      header: "Job ID",
    ...
    { key: "classification", header: "Classification", width: 170, sortable: true, render: (task) => <strong>{task.classification}</strong> },
    { key: "taskType", header: "Task Type", width: 120, sortable: true },
    { key: "commandType", header: "Command", width: 220, sortable: true },
    {
      key: "state",
      header: "State",
      width: 140,
    ...
    { key: "description", header: "Description", width: 300, sortable: true, className: "task-description-cell" },
    { key: "startTime", header: "Start Time", width: 150, sortable: true },
    { key: "endTime", header: "End Time", width: 150, sortable: true },
    { key: "orderedBy", header: "Ordered By", width: 120, sortable: true },
    {
      key: "actions",
      header: "Actions",
      width: 126,
      render: (task) => (
        <div className="task-row-actions" onClick={(event) => event.stopPropagation()}>
          <AppIconButton
            ...
            label={getTaskActionDisabledTitle("stop", task)}
            title={getTaskActionDisabledTitle("stop", task)}
            icon={<Square size={14} />}
            disabled={!canRunTaskAction("stop", task)}
            onClick={(event) => {
              event.stopPropagation();
              requestTaskAction("stop", task);
            }}
          />
          <AppIconButton
            variant="light"
            className="task-row-action-btn"
            label={getTaskActionDisabledTitle("cancel", task)}
            title={getTaskActionDisabledTitle("cancel", task)}
            icon={<X size={14} />}
            disabled={!canRunTaskAction("cancel", task)}
            onClick={(event) => {
              event.stopPropagation();
              requestTaskAction("cancel", task);
            }}
          />
          <AppIconButton
            variant="light"
            className="task-row-action-btn danger"
            label={getTaskActionDisabledTitle("delete", task)}
            title={getTaskActionDisabledTitle("delete", task)}
            icon={<Trash2 size={14} />}
            disabled={!canRunTaskAction("delete", task)}
            onClick={(event) => {
              event.stopPropagation();
              requestTaskAction("delete", task);
            }}
          />
        </div>
      ),
    },
  ], [pageStart, selectedTask?.id, sortConfig.key, sortConfig.direction]);
          ...
          <section className="settings-content d-grid gap-3 task-list-settings-content">
            <div className="settings-hero task-list-hero ema-panel-surface">
              <div>
                ...
                <h2>Task List</h2>
                <p>Monitor command jobs, execution state, endpoint delivery and operational actions.</p>
              </div>

              <div className="settings-score task-hero-score" aria-label="Task summary">
                ...
                <button type="button" className={`score-box task-score-box ${state === "Transferring" ? "active" : ""}`} onClick={() => setState("Transferring")}>
                  <span>Transferring</span>
                  <strong>{taskStats.transferring}</strong>
                  <small>Delivery in progress</small>
                </button>
                <button type="button" className={`score-box task-score-box ${state === "Transferred" ? "active" : ""}`} onClick={() => setState("Transferred")}>
                  <span>Completed</span>
                  <strong>{taskStats.transferred}</strong>
                  <small>{taskStats.completion}% average</small>
                </button>
              </div>
            </div>

            <section className="content-shell task-list-content-shell ema-panel-surface">
              <div className="content-head task-list-content-head">
                <div>
                  <span className="section-tag">TASK REGISTRY</span>
                  <h3>Command & Distribution Tasks</h3>
                  <p>Search, filter, export and inspect task execution from one registry.</p>
                </div>

                <div className="content-actions task-list-head-actions">
                  <button className="soft-btn" type="button" onClick={loadTasks} disabled={isLoading}>
                    {isLoading ? <Loader2 className="task-spin" size={15} /> : <RefreshCw size={15} />}
                    <span>{isLoading ? "Refreshing..." : "Refresh"}</span>
                  </button>
                  <button className="primary-btn" type="button" onClick={exportTasks} disabled={!filteredTasks.length}>
                    <Download size={15} />
              ...
              <div className="content-toolbar task-list-toolbar">
                ...
                <div className="task-list-filter-grid">
                  ...
                  <div className="task-toolbar-actions">
                    <button className="soft-btn task-reset-btn" type="button" onClick={resetFilters} disabled={!hasFilters}>
                      <X size={14} />
                      <span>Reset</span>
                    </button>
                    <button className="primary-btn task-apply-btn" type="button" onClick={loadTasks} disabled={isLoading}>
                      {isLoading ? <Loader2 className="task-spin" size={15} /> : <RefreshCw size={15} />}
                      <span>Apply</span>
                    </button>
                  </div>
              ...
              <div className="content-body task-list-content-body">
                {errorMessage && (
                ...
                <div className="settings-helper-card compact task-table-meta-bar">
                  <div>
                    <strong>{filteredTasks.length} records</strong>
                    <span>Showing {pageStart}-{pageEnd} from filtered Task List results.</span>
                  </div>

                  <label className="task-page-size-control">
                    <span>Rows</span>
                ...
                <AppTable
                  ...
                  onRowClick={(task) => openTaskDetails(task.id)}
                  emptyTitle="No task found"
                  emptyDescription="Try changing classification, state, date, or search keyword."
                />
                ...
                <AppPagination
                  ...
                  showPageSize={false}
                  disabled={isLoading}
                  className="task-pagination-compact"
  ...
  return (
    <div className="task-detail-modal-layer" onClick={onClose} role="presentation">
      <section className="task-detail-modal ema-panel-surface" role="dialog" aria-modal="true" aria-label={`Task ${task.id} detail`} onClick={(event) => event.stopPropagation()}>
        ...
        <div className="task-detail-modal-body task-detail-workspace">
          <div className="task-detail-summary-grid">
            <section className="task-card task-detail-action-card">
          ...
          <section className="task-card task-detail-data-card task-target-status-card">
            <div className="task-detail-data-head">
              <div>
                <h3>Target Endpoint List</h3>
                <p>Assigned targets and individual endpoint status are shown together here.</p>
              </div>

              <div className="task-detail-data-controls task-target-status-controls">
                <div className="task-target-status-counts" aria-label="Target row count">
                  <span>Targets <b>{targets.length}</b></span>
                </div>
              </div>
            </div>

            <div className="task-detail-data-body" role="region" aria-label="Target endpoint status table">
              <TaskTargetList
                task={task}
                targets={targets}
                progressDetails={progressDetails}
